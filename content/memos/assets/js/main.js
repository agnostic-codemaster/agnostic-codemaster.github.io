    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].resourceList && data[i].resourceList.length > 0) {
            ...
            for (var j = 0; j < resourceList.length; j++) {
                ...
                if (resType == 'image') {
                    imgUrl += '<img loading="lazy" src="' + memos + 'o/r/' + '/' + resourceList[j].name + '"/>'
                    resImgLength = resImgLength + 1
                }
                if (resType !== 'image') {
                    resUrl += '<a target="_blank" rel="noreferrer" href="' + memos + 'o/r/' + '/' + resourceList[j].name + '">' + resourceList[j].filename + '</a>'
                }
            }
            if (imgUrl) {
                var resImgGrid = ""
...
function getTotal() {
    var totalUrl = memos + "api/v1/memo/amount?creatorId=" + memo.creatorId
